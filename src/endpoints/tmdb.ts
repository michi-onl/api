...
export class TmdbTrending extends OpenAPIRoute {
  ...
  async handle(c: AppContext) {
    ...
    const data = await cached(
      ...
      () => fetchTrending(token),
      (result) =>
        result.movies.data.length > 0 || result.tv_shows.data.length > 0,
    );
    return c.json(data);
  }
}
...
async function fetchCategory(type: "movie" | "tv", token: string) {
  ...
  if (!res.ok) {
    console.log(`TMDB ${type} fetch failed: ${res.status}`);
    throw new Error(`TMDB ${type} fetch failed: ${res.status}`);
  }

  const json = (await res.json()) as Record<string, unknown>;
  if (!Array.isArray(json.results)) {
    console.log(`Unexpected TMDB ${type} response: missing results array`);
    throw new Error(`Unexpected TMDB ${type} response: missing results array`);
  }
  const items = (json.results as TmdbResult[])
    .slice(0, MAX_ITEMS)
    .map((item, i) => {
